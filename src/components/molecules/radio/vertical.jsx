...
export default function RadioVertical({
  ...
  titleClassName,
  fieldClassName,
  formControlLabelStyle = {}, // Renamed the prop to target FormControlLabel styles
  ...restProps
}) {
            ...
            ? options
              ...
              .map((item) => (
                <FormControlLabel
                  ...
                  control={
                    <Radio sx={{ "&.Mui-checked": { color: "#00529C" } }} />
                  }
                  label={item.label}
                  sx={{ ...formControlLabelStyle }} // Applied custom styles here
                />
              ))
            : "You must provide options props"}
        </RadioGroup>
        <FormHelperText sx={{ fontSize: "16px", marginLeft: "0" }}>
